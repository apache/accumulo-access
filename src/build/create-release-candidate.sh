...
createEmail() {
  ...
  local srcSha
  [[ -n $4 ]] && srcSha=$4 || srcSha=$(prompter 'SHA512 for source release zip file' '[0-9a-f]{128}')
...
Source (official release artifact): $(green "$stagingRepoPrefix-$stagingrepo/org/apache/$tlpName/$projName/$ver/$projName-$ver-$srcQualifier.zip")
...
createReleaseCandidate() {
  ...
  local numSrc
  numSrc=$(find target/checkout/ -type f -name "$projName-$ver-source-release.zip" | wc -l)
  shopt -s globstar
  local srcSha
  srcSha=""
  [[ $numSrc == "1" ]] && srcSha=$(sha512sum target/checkout/**/"$projName-$ver-source-release.zip" | cut -f1 -d" ")
